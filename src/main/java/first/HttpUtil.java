...
public class HttpUtil {
    ...
    private static Post getLastPost(User user) throws IOException, InterruptedException {
        HttpRequest request = HttpRequest.newBuilder()
        ...
        List<Post> posts = GSON.fromJson(responsePosts.body(),new TypeToken<List<Post>>(){}.getType());
        return Collections.max(posts, Comparator.comparingInt(Post::getId));
    }

    public static List<Task> getAllOpenedTasksOfUser(User user) throws IOException, InterruptedException {
        HttpRequest request = HttpRequest.newBuilder()
